...
from audio_processing.convert_audio import convert_wav_to_mp3, convert_wav_to_pcm
from audio_processing.process_audio import (
    prepare_segment_data,
    process_audio_file,
    split_audio_by_segments,
)
from models.download_model import check_and_download_model
from audio_processing.speaker_identify import (
...
from utils.helpers import delete_audio_segments, set_custom_vosk_config, set_default_vosk_config
...
from audio_processing.save_results import save_filtered_dialogue, save_summary, save_transcription_results
...
CLEAR_AUDIO_PARTS_RUN = os.getenv("CLEAR_AUDIO_PARTS_RUN", "False").lower() == "true"
ASSIGNSPEAKERS = os.getenv("ASSIGNSPEAKERS", "False").lower() == "true"
FILTER_UNNECESSARY_RTTM = os.getenv("FILTER_UNNECESSARY_RTTM", "True").lower() == "true"
MIN_RTTM_DURATION = float(os.getenv("MIN_RTTM_DURATION", 2.0))
USE_CUSTOM_VOSK = os.getenv("USE_CUSTOM_VOSK", "False") == "True"
USE_BATCHES = os.getenv("USE_BATCHES", "False").lower() == "true"
BATCH_SIZE = float(os.getenv("BATCH_SIZE", 10.0))  # Default batch size in seconds

# File paths from .env
AUDIO_FILE_NAME = Path(os.getenv("AUDIO_FILE_NAME"))
WORKSPACE_ENV = Path(os.getenv("WORKSPACE"))
...
OUTPUT_DIR_PARTS = os.getenv("OUTPUT_DIR_PARTS")
...
def batch_segments(rttm_segments, batch_size):
    ...
    for batch_num, batch in enumerate(batched_segments):
        ...
        audio_segment_file_path = (
            output_dir / f"{AUDIO_FILE_NAME.stem}_converted_{batch['speaker']}_part{batch_num + 1}.wav"
        )
...
def check_and_process_audio(audio_file, model_path, output_dir):
    ...
    rttm_file = os.path.join(output_dir, f"{os.path.basename(audio_file)}.rttm")
    json_file = os.path.join(output_dir, f"{os.path.basename(audio_file)}_transcription.json")
...
def process_audio_segments(segment_data):
    ...
    try:
        ...
        model = Path(VOSK_MODEL_FULL_PATH_ENV)
        # transcription = process_audio_file(str(audio_segment_file), model, 1, "./output")
...
def main():
    ...
    model_path = check_and_download_model(model_name, str(VOSK_MODEL_FULL_PATH_ENV))
    ...
    segment_data_list = prepare_segment_data(
        rttm_segments, str(VOSK_MODEL_FULL_PATH_ENV), total_audio_length, f"{audio_name}_converted"
    )
    ...
    if not DRY_RUN:
        ...
        save_transcription_results(speaker_transcription, csv_file, json_file, rttm_file)
